...
DNS_Header_t* parse_header(const char* bytes){
	return (DNS_Header_t*)bytes;
}
...
size_t parse_question(const char* bytes,DNS_Question_t* question){
	...
	while(q_start[++i] != (char)0){
		std::cout << "Sup";
		std::cout << "Length is: " <<  std::dec << len;
		std::string name;
		for(int j = 0; j<len;j++){
	...
	char delimiter = '.';
	std::string domain_name_str = std::accumulate(std::next(names.begin()), names.end(),
        names[0], [&delimiter](std::string &acc, const std::string &s) {
...
static std::string dns_class_strings[] = {
"NULL",
"IN",
"CS",
"CH",
